...
```{r}
# DATABASE CONNECTION STORED HERE -- set up connection with DBI and RSQLite
...
left_join(flights_db,by = "carrier") %>% 
  filter(month == 1 & day == 1) %>% 
  select(carrier, name, flight)

#b
jan_flights_local <- airlines_db %>% collect()
jan_flights_local
```
...
ALWAYS DISCONNECT !!!!!!!!! When you are done with your SQL queries, it is a good idea to disconnect from the database. This becomes increasingly important if you work with servers that charge you for their services!
